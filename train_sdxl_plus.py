...
class IPAdapter(torch.nn.Module):
    ...
    def forward(self, noisy_latents, timesteps, encoder_hidden_states, unet_added_cond_kwargs, image_embeds, added_params):
        ...
        # New added
        batch_size, num_objects, dim = image_embeds.shape[0], image_embeds.shape[1], image_embeds.shape[3]
        image_embeds = image_embeds.reshape(batch_size * num_objects, -1, dim)
    ...
    for epoch in range(0, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(ip_adapter):
                ...
                with torch.no_grad():
                    # New added
                    num_objects, entity_size = clip_images.shape[1], clip_images.shape[-1]
                    clip_images = clip_images.reshape(bsz * num_objects, 3, entity_size, entity_size)
                    ...
                    image_embeds = image_embeds.reshape(bsz, num_objects, -1, image_embeds.shape[-1])
            ...
            if global_step % args.validation_steps == 0 or global_step == stop_step:
                ...
                inference_on_concept101_dataset(accelerator, args.pretrained_model_name_or_path, state_dict, None, args.image_encoder_path, output_view_dir, args.eval_data_root, args, num_tokens=num_tokens, scale=args.infer_scale, is_plus=True)
